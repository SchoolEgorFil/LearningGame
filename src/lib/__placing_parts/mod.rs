...
impl Plugin for PlayerPlacingPlugin {
    ...
    fn build(&self, app: &mut bevy::prelude::App) {
        ...
        .add_systems(OnEnter(GameState::GameRunning), setup)
        .add_systems(
            Update,
            (
...
pub fn setup(
    ...
    let image: Handle<Image> = asset_server.load("internal/textures/placement_grid.png");
    let emissive: Handle<Image> = asset_server.load("internal/textures/placement_grid_emissive.png");
    ...
    for placement_event in placement_ev_r.iter() {
        match placement_event.object {
            PlacingObject::LaserMirror => {
                let m = asset_server.load("internal/components/laser_mirror.glb#Scene0");
                commands.spawn((
                    SceneBundle {
                        scene: m,
            ...
            PlacingObject::LaserPointer => {
                let m = asset_server.load("internal/components/laser_pointer.glb#Scene0");
                commands.spawn((
                    SceneBundle {
                        scene: m,
                        transform: placing_res.placement.clone().unwrap(),
